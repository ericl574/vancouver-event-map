...
export default function EventCard({ event, isSelected, onClick, authSession, showActions = true }) {
      ...
      >
        ...
        <div className="relative h-24 w-full overflow-hidden bg-slate-100">
          ...
          <div className="absolute left-3 top-3">
            <span className="rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest backdrop-blur-sm" style={{ color: category.hex }}>
              Featured
            </span>
          </div>
        </div>
        ...
        <div className="px-3.5 pb-2.5 pt-2.5">
          <div className="mb-0.5 text-[10px] font-bold uppercase tracking-widest" style={{ color: category.hex }}>
            {category.label}
          </div>
          ...
          <p className="mt-0.5 text-xs font-semibold text-slate-700">
            ...
            {event.price && (
              <span className="ml-1 font-normal text-slate-500">· {event.price}</span>
            )}
          </p>
          ...
          {showActions && <div className="mt-2 flex items-center justify-between border-t border-slate-100 pt-2">
            <div className="flex items-center gap-1">
              <button
                ...
                className={`flex h-7 w-7 items-center justify-center rounded-full border transition ${
                  isSaved
                    ...
                    : "border-slate-200 bg-slate-50 text-slate-500 hover:border-pink-200 hover:text-pink-500"
                }`}
              >
              ...
              <button
                ...
                onClick={handleShare}
                title="Share"
                className="flex h-7 w-7 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-500 transition hover:border-slate-300 hover:text-slate-700"
              >
            ...
            {eventUrl ? (
              <button
                ...
                className="rounded-full bg-pink-500 px-3.5 py-1.5 text-xs font-black text-white shadow-sm transition hover:bg-pink-600"
              >
                Tickets
              </button>
            ) : event.isFree ? (
  ...
  return (
    <button
      type="button"
      onClick={() => onClick(event)}
      className="w-full overflow-hidden rounded-2xl border border-slate-200 bg-white/95 text-left transition hover:border-slate-300 hover:bg-slate-50"
    >
