...
- Event-list action buttons should be understandable, not mystery icons, when the reference shows labels. Use icon+text labels such as `Save`, `Share`, `Add to calendar`, and `Tickets` on the featured/selected card; compact cards may use a simpler bookmark affordance.
...
- Event list featured/selected card should match the client-approved reference when provided: square rounded image on the left, featured badge over image, text on the right, bookmark icon at top-right, and no bottom action row unless explicitly requested.
