...
- Map event marker/icon colors should use the approved event-map palette when updating marker visuals: `#EFD55E`, `#89D0C2`, `#F77A82`, `#87AACB`, `#077ABD`. Use these colors mainly for marker centers/icons while keeping a white marker body/ring and subtle shadow for contrast. If yellow or mint become too soft on the live map, darken only those slightly while preserving the same palette direction. Preserve marker behavior while mapping event categories into this palette.
