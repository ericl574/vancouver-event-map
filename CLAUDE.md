...
Past events should be archived/hidden from the main discovery map rather than immediately hard-deleted. Use event end time when available, otherwise start date/time plus a reasonable grace period. Keep records available for saved/detail/history/admin needs unless the client explicitly requests permanent deletion.

When reimporting Ticketmaster events, prefer an idempotent one-year import workflow. Fetch a full year of Vancouver/Greater Vancouver events, but avoid duplicate rows by upserting on stable Ticketmaster/source IDs first and then falling back to normalized title + venue + event date/time matching. Re-running the import should update existing events, not create repeated copies.
Do not treat recurring performances as duplicates when they have different dates or start times. Same title and same venue can be multiple valid events, especially theatre/sports/touring shows. Deduplicate only exact same source ID or same normalized title + venue + same event date + same start time.
When the UI shows apparent duplicate events, diagnose whether they are true duplicates, source variants, or legitimate separate showtimes before deleting/merging. Compare source IDs, normalized title, venue, date, start time, ticket URL, image URL, and source name. Near-duplicate music events with same artist/title/venue/date but slightly different title formatting or conflicting times should be flagged for review or merged only when confidence is high.
...
- The search/filter area should include a small, unobtrusive hint near the filter button explaining the default visible time window, e.g. `Default: 1 month`, so users understand why the event count is lower than the total database count.
- Do not label the search submit/enter button as `Near`; that is confusing with `Near Me`. Use clearer search action text such as `Search`, `Go`, or an icon-only submit affordance.
...
- The `Near Me` / nearest feature should not display a persistent explanatory sentence/pill such as `Nearest: [event] · X km away` on the screen. It may update the map/list/selection and show errors only when needed, but avoid extra nearest status text cluttering the UI.
...
- When the client asks to make the event list, search bar, and time filter page `1/15 larger`, increase all desktop left-side surface widths together by about 6.7% so they stay aligned. Adjust text/button spacing and font sizes only as needed to prevent wrapping or crowding.
...
- Selected/featured event list cards should not have an internal divider line between the image/content area and the bottom action buttons. Keep the outer pink frame/border, but remove any internal pink/gray border separating picture/content from buttons unless explicitly requested.
...
- In the right-side event preview/detail card, the event description/details text should be fully visible and wrap naturally. Do not truncate with ellipses or line clamps in the detail card. List cards may still use truncation/clamping where needed.
...
- Map event marker/icon colors and related UI accents should use the client's approved stronger palette when specified: blue `#1355B2`, big grouped number bubble pink `#F5569B`, Nightlife `#0D21A5`, Festivals `#701D1C`, Arts `#FFC872`, and Food & Drink `#E9631A`. The client also provided `FFXBEB` for pink, but that is not a valid hex value; clarify before implementing that exact pink or use the nearest confirmed intended pink only with approval. Preserve marker behavior while mapping event categories into this palette, and keep white marker body/ring plus subtle shadow for contrast.
...
- When an event is selected from the map by clicking its marker, the left event list should scroll/jump to that event's card and highlight/select it when the event is present in the current list context. Map selection and list preview position should stay synchronized.
